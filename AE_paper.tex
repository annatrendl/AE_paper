...
\AT{Imagine that you are in a restaurant, and after a nice meal, you are considering the two dessert options available on the menu, cheesecake or pecan pie. You are torn between the two, as in one minute you think about the creamy texture of the cheesecake, and the in the next minute you think about the rich, nutty flavour of the pecan pie. As the waiter approaches to take your order, he informs you that a third dessert option, apple pie is also available today. You find the taste of apple pie to be quite bland in general, and you instantly know that you prefer the original two options over it. How does the addition of the apple affect your dessert choice?}

\AT{Decades on decision-making research suggests that the presence of the apple pie (decoy) in the choice set makes you more likely to choose the pecan pie (target) over the cheesecake (competitor). This phenomenon is called the attraction effect (also known as asymmetric dominance effect), and it is one of the most widely researched decision biases in consumer decision making. In essence, it states that when the decision maker is indifferent between the target and the competitor (pecan pie and cheesecake in the example), the addition of an inferior decoy option that resembles the target (apple pie is similar to pecan pie, but is slightly less liked by the decision maker) increases the likelihood that the target will be chosen.}

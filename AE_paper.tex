...
The attraction effect is important, because it poses a challenge to all choice models that rely on the assumption that preferences can be represented on a cardinal scale in the form of utilities (this is also called simple scalability, one of the consequences of Luce's Choice Axiom; \citeNP{Luce1959}). The attraction effect represents a violation of independence of irrelevant alternatives, which requires that the preference ranking of two options should not be affected by adding new options to the choice set. In addition, it violates the axiom of regularity, which states that by extending the choice set, an option's choice probability cannot increase (\citeNP{Luce1977}; \citeNP{Tversky1972a}). Therefore, the existence of the attraction effect means that preferences for one option cannot be represented by a single internal magnitude that is invariant to the other options' value in the choice set.

Recently, the relevance of the attraction effect in real-world choices became a contentious issue in the decision-making literature. The first multiattribute choice experiments demonstrating the attraction effect (e.g., Huber, Payne, and Puto, 1982; Simonson \& Tversky, 1982; Tversky, 1972) often used stimuli presented as a set of numerical attributes (e.g., cars presented as numerical values for gas mileage and ride quality). However, recent experiments in multiattribute choice introduced what can perhaps be considered as more ecologically valid stimuli (e.g., movies presented as thumbnails, and titles on Netflix or photographs of popular snacks). These experiments reported difficulties in replicating choice patterns that are consistently present in choices with numerical attributes.

...
\AT{In the attraction effect, adding a third option to a choice set of two options can reverse the preference for the original options, and even increase the choice share of one of the original options. This constitutes a violation of the axioms of regularity and independence from irrelevant alternatives, which are core properties of any choice model in which the utility of each option is stable across choice sets. Consequently, the attraction effect has driven the development of a set of influential models of multiattribute choice in the past 20 years. However, recently, \citeA{Frederick2014} claimed that the effect is limited to choices where options have numerical attributes and is absent from real-world consumer choices with naturalistic options (e.g., snacks, movies), a claim which would severely undermine its theoretical importance. \citeA{Huber2014} criticised \citeauthor{Frederick2014}'s experiments, laying down a set of criteria that should be met by any experiment wishing to test for the attraction effect in real-world consumer choices. This article presents the first experiment that meets these criteria. The results show a precisely zero attraction effect.}
%194 words now
\end{abstract}
%Keywords
...
\AT{Should I only upload the cleaned version of the choice data and the code that produces the results in the paper, or should I upload everything, including the movie stimuli, the code creating the quadruplets, the code that creates bespoke quadruplets; the ratings, choice data and similarity data in csv and the code that cleans the data?}
...
7. Title. Style checklist. Abstract needs to be shorter. Abstract should be in 3rd person (“the authors”). I do not understand the requirement about the headings.
